...
#GenomicAlter$KIT[which(GenomicAlter$chr4q12.Gain==1)] <- 1
...
#GenomicAlter <- GenomicAlter[,c("AID","KIT","MAPK","PI3K","chr1p36.Loss",
#                                "chr2p24.Gain","chr3p23.Gain","chr4q.Loss","chr6q25.Loss","chr8q13.Gain","chr11q23.Loss","chr12p.Gain",
#                                "chr13q.Loss","chr16p13.3.Loss","chr20q.Gain", "chr21q.Gain","chr22q11.Gain","chrXq.Gain","chrXq.Loss", "WGD")]
GenomicAlter <- GenomicAlter[,c("AID","KIT","KRAS","RRAS2","MAPK","PI3K","chr1p36.Loss","chr1q21.Gain", 
                                "chr2p24.Gain","chr3p23.Gain","chr4q12.Gain","chr4q.Loss","chr6q25.Loss","chr8p.Loss","chr8p.Gain","chr8q13.Gain","chr11q23.Loss","chr12p.Gain",
                                "chr13q.Loss","chr16p13.3.Loss","chr20q.Gain", "chr21q.Gain","chr22q11.Gain","chrXq.Gain","chrXq.Loss", "WGD")]
...
myTable$Dotcolor <- "NS"
myTable$Dotcolor[which(myTable$OddsR > 0 & myTable$qValue <= 0.05)] <- "Cooccur" 
myTable$Dotcolor[which(myTable$OddsR < 0 & myTable$qValue <= 0.05)] <- "Exclusive" 
...
coMu.plot<-coMu.plot+scale_fill_gradientn(name="Odds ratio\n(log2)",colours=c('#4d9221','#b8e186','white','#f1b6da','#c51b7d'),limits=c(-6.8,6.8),breaks=seq(-6,6,3))
coMu.plot<-coMu.plot+scale_x_discrete(position = "top")

plot1 <- coMu.plot

figure_1<-rbind(ggplotGrob(plot1),size="last")
ggsave(file="./Clinical_Mutation_figure3.pdf", plot=figure_1,bg = 'white', width = 22.2, height = 13.7, units = 'cm', dpi = 600)
