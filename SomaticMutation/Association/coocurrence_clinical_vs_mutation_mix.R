...
#######
Deconvolution <- read.table("../../Transcriptome_subtyping/LOOCV_noMix/CIBERSORTx_Job9_Results.txt",header = T)

Germinoma <- rep(NA,nrow(Interaction))
Germinoma[which(Interaction$AID %in% Deconvolution$Mixture[which(Deconvolution$Germinoma<0.235)] )] <- 0
Germinoma[which(Interaction$Histology != "Mix" | Interaction$AID %in% c("NG11"))] <- 0
Germinoma[which(Interaction$Histology != "xxx")] <- 0
Germinoma[which(Interaction$Histology == "Germinoma")] = 1

#Germinoma[which(Interaction$Histology != "Germinoma" & Interaction$Histology != "Mix" )] = 0

YST <- rep(NA,nrow(Interaction))
YST[which(Interaction$AID %in% Deconvolution$Mixture[which(Deconvolution$YST<0.235)] )] <- 0
YST[which(Interaction$Histology != "Mix" )] <- 0
YST[which(Interaction$Histology != "xxx")] <- 0
...
Teratomas[which(Interaction$AID %in% Deconvolution$Mixture[which(Deconvolution$Teratoma<0.235)] )] <- 0
Teratomas[which(Interaction$Histology != "Mix" )] <- 0
Teratomas[which(Interaction$Histology != "xxx")] <- 0
Teratomas[which(Interaction$Histology == "Teratoma" | Interaction$Histology == "IMT")] = 1
#Teratomas[which(Interaction$Histology != "Teratoma" & Interaction$Histology != "IMT" & Interaction$Histology != "Mix" & Interaction$Histology != "IMT" )] = 0
...
coMu.plot<-coMu.plot+scale_size(name   = "Fisher's test\n   q-value", range = c(2,7.5),transform = "exp",  breaks = c( 1,1.3,2,3,4,5,6,7,8),labels = expression(0.1,0.05,10^-2,10^-3, 10^-4,10^-5, 10^-6,10^-7,10^-8),guide = guide_legend(nrow=2))
...
ggsave(file="./Clinical_Mutation_figure6.pdf", plot=figure_1,bg = 'white', width = 22.9, height = 13.5, units = 'cm', dpi = 600)
