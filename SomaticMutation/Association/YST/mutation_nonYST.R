...
SV <- read.table("../../Landscape/results/Final_ComplexSV.txt",header=T)
...
Interaction <- merge(Interaction,SV)
Interaction <- merge(Interaction,Number)


Interaction <- Interaction[which(Interaction$Histology!="YST" & Interaction$Domin.His!="YST"),]
AID <- Interaction$AID
...
plot_1.data<-myTable
plot_2.data <- plot_1.data[which(plot_1.data$Frac < 0.4),]
plot_3.data <- plot_1.data[which(plot_1.data$Frac >= 0.4),]
...
coMu.plot<-coMu.plot+geom_bar(data = plot_3.data,aes(x=reorder(Mut,-Frac),y=Frac),alpha=0.9,fill="#b2182b",size=0.6,width=0.5,stat='identity',position=position_dodge2()) 
...
coMu.plot<-coMu.plot+theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(1,6,1,1),'lines'),plot.title=element_text(size=24,vjust=0.5,hjust=0.5,face='bold.italic'),
...
coMu.plot<-coMu.plot+scale_x_discrete(position = "bottom")+scale_y_continuous(expand = c(0,0),limits = c(0,0.7))

plot1 <- coMu.plot

figure_1<-rbind(ggplotGrob(plot1),size="last")
ggsave(file="./ranking_nonYST.pdf", plot=figure_1,bg = 'white', width = 20, height = 9, units = 'cm', dpi = 600)
