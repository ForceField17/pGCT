...
#CNV versus methylation
################
CNV <- read.table("../../Pancancer_L1/Final_CNV.txt",header = T)
...
Cancers <- c(TCGA)
L1methy <- rep(NA,length(Cancers))
L1HSmethy <- rep(NA,length(Cancers))
myTable <- {}
for(i in c(1:length(Cancers))){
  ...
  tmpMethy$AID <- gsub("\\.","-",tmpMethy$Sample_ID)
  tmpMethy$cancerType <- Cancers[i]
  tmp <- merge(CNV,tmpMethy)
  myTable <- rbind(myTable ,tmp)
}
myTable <- myTable[,c(1,6,5,7,8,9)]
write.table(myTable ,file = "../TCGA_samples_with_methy_and_CNV.txt",quote = F,row.names = F,sep = "\t")

xxx <- aov(MeanL1HSmethy ~ cancerType * chr1p36,data = myTable)
summary(xxx)
# 计算偏 η² (partial eta-squared)，这是多因素ANOVA最常用的指标
eta_squared(xxx, partial = TRUE)

yyy <- aov(MeanL1methy ~ cancerType * chr1p36,data = myTable)
summary(yyy)
# 计算偏 η² (partial eta-squared)，这是多因素ANOVA最常用的指标
eta_squared(yyy, partial = TRUE)


##############
#Ntable <- as.data.frame(table(myTable$cancerType));colnames(Ntable) <- c("cancerType","N")
#myTable <- myTable[which(myTable$cancerType %in% Ntable$cancerType[which(Ntable$N>=20 )]),]
...
p_values <- myTable_filtered %>%
  group_by(cancerType) %>%
  wilcox_test(MeanL1HSmethy ~ chr1p36) %>%
  ...
  mutate(
    ...
    p_y_pos = max(myTable_filtered$MeanL1HSmethy[myTable_filtered$cancerType == cancerType]) * 1.08
  ) %>%
...
cancer_order <- myTable_annotated %>%
  group_by(cancerType) %>%
  summarise(median_methy = median(MeanL1HSmethy, na.rm = TRUE)) %>%
...
loss_color <- "#2166ac"   # 红色 (loss)
  ...
  # 绘制主图
  p <- ggplot(myTable_annotated, aes(x = cancerType, y = MeanL1HSmethy, color = chr1p36)) +
    # 抖动散点（半透明，避免重叠过多）
    ...
    # 可以加入一条水平线标注整体中位数
    geom_hline(yintercept = median(myTable_annotated$MeanL1HSmethy, na.rm = TRUE), 
               linetype = "dashed", color = "gray30", linewidth = 0.3, alpha = 0.5)
  
  # 显示图形
  AllTumorure_2<-rbind(ggplotGrob(p),size="last")
  ggsave(file="Wilcon_TCGA_methy_L1HS.pdf", plot=AllTumorure_2,bg = 'white', width = 24, height = 8, units = 'cm', dpi = 600)
  

xxx <- aov(MeanL1HSmethy ~ cancerType * chr1p36,data = myTable_filtered)
summary(xxx)
eta_squared(xxx, partial = TRUE)

yyy <- aov(MeanL1methy ~ cancerType * chr1p36,data = myTable_filtered)
summary(yyy)
eta_squared(yyy, partial = TRUE)

myRank <- data.frame(aggregate(myTable_filtered$MeanL1HSmethy, list(myTable_filtered$cancerType), FUN=mean))
L1pro <- data.frame(aggregate(myTable_filtered$MeanL1methy, list(myTable_filtered$cancerType,myTable_filtered$chr1p36), FUN=mean))
colnames(L1pro) <- c("Cancer","chr1p36","L1methy")
matrix_result <- acast(L1pro, Cancer~chr1p36 ,value.var = "L1methy")
cohort <- myRank$Group.1[order(myRank$x,decreasing = T)]
matrix_result <- matrix_result[c(cohort[grep("TCGA",cohort)]),]

pdf(file = paste0("./heatmap_CNV_versus_cancer_L1pro.pdf"),width=6,height=4)
pheatmap(t(matrix_result),cluster_cols = F,cluster_rows = F,treeheight_col = 30,
         ...
         show_rownames = T,annotation_legend = T,cellheight=10,
         scale = "none")
dev.off()

##############
L1pro <- data.frame(aggregate(myTable_filtered$MeanL1HSmethy, list(myTable_filtered$cancerType,myTable_filtered$chr1p36), FUN=mean))
colnames(L1pro) <- c("Cancer","chr1p36","L1HSmethy")
matrix_result2 <- acast(L1pro, Cancer~chr1p36 ,value.var = "L1HSmethy")
cohort <- myRank$Group.1[order(myRank$x,decreasing = T)]
matrix_result2 <- matrix_result2[c(cohort[grep("TCGA",cohort)]),]
colnames(matrix_result2) <- c("L1HS_1p36loss","L1HS_1p36WT")
matrix_result3 <- cbind(matrix_result,matrix_result2)
pdf(file = paste0("./heatmap_CNV_versus_cancer_L1HSpro.pdf"),width=6,height=4)
pheatmap(t(matrix_result3),cutree_rows= 2,cluster_cols = F,cluster_rows = F,treeheight_col = 30,
         color = colorRampPalette(c("#000099","#0000CC","#0000FF","#3333FF","#6666FF","#9999FF","#CCCCFF", "grey95","#FFCCCC","#FF9999","#FF6666","#FF3333", "#FF0000","#CC0000","#990000"))(50),
         annotation_names_row=F,# gaps_col = c(3,14,48,52,57), 
         annotation_names_col=F, gaps_row = c(2),
         show_rownames = T,annotation_legend = T,cellheight=10,
         scale = "none")
dev.off()
...
xxx <- aov(RT_burden ~ subtype * chr1p36,data = RT)
summary(xxx)
eta_squared(xxx, partial = TRUE)

mergeTable <- merge(RT,myTable_filtered)
...
ggsave(file="Volcano_TCGA_1p36.pdf", plot=AllTumorure_2,bg = 'white', width = 11, height = 8, units = 'cm', dpi = 600)
